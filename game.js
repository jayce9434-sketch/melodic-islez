...
const RESOURCE_GAMES={
  coinGarden:{name:'Coin Garden',icon:'🌼',currency:'coins',rewardIcon:'🪙',tag:'EASY COIN GAME',limit:5,windowMs:ONE_HOUR,unlock:'groove',desc:'The chill alternative to Coin Rush. You get 4.5 seconds and wrong taps do not end the round—just find the monster holding the glowing coin.',bestLabel:'Best coins'},
  gemMine:{name:'Gem Mine',icon:'💎',currency:'gems',rewardIcon:'💎',tag:'GEM GAME',limit:5,windowMs:ONE_HOUR,unlock:'groove',desc:'Memorize the glowing crystal, then pick it from the shuffled mine. Good memory = free Gems.',bestLabel:'Best gems'},
  starfall:{name:'Starfall Observatory',icon:'⭐',currency:'stars',rewardIcon:'⭐',tag:'STAR GAME',limit:5,windowMs:ONE_HOUR,unlock:'groove',desc:'Study tiny constellations and choose the matching pattern after the sky goes dark.',bestLabel:'Best stars'},
  noteJam:{name:'Note Jam',icon:'🎵',currency:'notes',rewardIcon:'🎵',tag:'RHYTHM GAME',limit:5,windowMs:ONE_HOUR,unlock:'groove',desc:'Tap on the beat as the note crosses the sweet spot. Perfect timing showers you with Music Notes.',bestLabel:'Best notes'}
};
...
function freshState(){
  return {
    ...
    arcade:{
      coinGarden:{runs:[],best:0,total:0},
      gemMine:{runs:[],best:0,total:0},
      starfall:{runs:[],best:0,total:0},
      noteJam:{runs:[],best:0,total:0}
    },
    stats:{collectedCoins:0,breedCount:0,decorBought:0,songsPlayed:0,instantBreeds:0,notesSpent:0,starsSpent:0,gemsSpent:0,monstersBought:0,masteryBought:0,hatched:0,activePlayMs:0,coinRushRuns:0,coinRushCoins:0,arcadeRuns:0,coinGardenRuns:0,gemMineRuns:0,starfallRuns:0,noteJamRuns:0},
...
let state=load();
let audio=null,masterGain=null,musicBus=null,sfxBus=null,songTimer=null,songStep=0,selectedBreed=[],collectionVariant='All',lastActiveTick=now(),rushGame=null,arcadeGame=null,arcadeRAF=null;
...
function migrate(raw){
  ...
  Object.keys(s.breedJobs).forEach(k=>{const j=s.breedJobs[k];if(j&&Number(j.endsAt)>now()+10*60000){j.endsAt=now()+10*60000;j.startedAt=Math.min(Number(j.startedAt)||now(),now());}});
  ...
  s.arcade={...base.arcade,...(raw.arcade||{})};Object.keys(RESOURCE_GAMES).forEach(k=>{s.arcade[k]={...base.arcade[k],...(s.arcade[k]||{})};s.arcade[k].runs=Array.isArray(s.arcade[k].runs)?s.arcade[k].runs.filter(x=>Number.isFinite(Number(x))).map(Number):[];});
...
function demoState(){
  ...
  s.stats={...s.stats,collectedCoins:98000,breedCount:18,decorBought:9,songsPlayed:14,instantBreeds:3,notesSpent:740,starsSpent:11,gemsSpent:14,monstersBought:25,masteryBought:5,hatched:16,activePlayMs:76*60000,coinRushRuns:7,coinRushCoins:58400,arcadeRuns:11,coinGardenRuns:4,gemMineRuns:3,starfallRuns:1,noteJamRuns:3};
  s.coinRush={runs:[now()-52*60000,now()-36*60000],best:{easy:4800,medium:7920,hard:13840,extreme:22600},totalEarned:58400};
  s.arcade={coinGarden:{runs:[now()-41*60000],best:6420,total:18840},gemMine:{runs:[],best:12,total:26},starfall:{runs:[],best:4,total:7},noteJam:{runs:[now()-18*60000],best:310,total:740}};
...
function init(){
  ...
  setInterval(()=>{ensureRotationState();trackActivePlayTime();updateTimers();renderTop();if(!$('#modalLayer').classList.contains('hidden')){if($('#modalTitle').textContent==='Quests')renderQuests();else if($('#modalTitle').textContent==='Coin Rush'&&!(rushGame&&rushGame.active))updateRushMenuTimer();else if($('#modalTitle').textContent==='Resource Arcade'&&!(arcadeGame&&arcadeGame.active))updateArcadeMenuTimers();}},1000);
  ...
  if(demoMode){if(island&&ISLANDS[island])state.currentIsland=island;renderAll();if(screen&&['market','breed','collection','quests','settings','map','mastery','coinrush','arcade'].includes(screen))setTimeout(()=>openModal(screen),100);}
}
function bindStatic(){
...
function openModal(type){
  ...
  if(arcadeGame&&arcadeGame.active&&type!=='arcade')endResourceGame(false,true);
  stopSong();$('#modalLayer').classList.remove('hidden');$$('.bottom-nav button').forEach(b=>b.classList.toggle('active',b.dataset.nav===type||(type==='coinrush'&&b.dataset.nav==='arcade')));
  const titles={market:['MONSTER MARKET','Market'],breed:['HARMONY HATCH','Breeding'],collection:['MONSTER BOOK','Collection'],quests:['GOALS','Quests'],coinrush:['COIN RUSH','Coin Rush'],arcade:['RESOURCE ARCADE','Resource Arcade'],settings:['OPTIONS','Settings'],map:['WORLD','Island Map'],mastery:['STAR WORKSHOP','Island Mastery']};const [eye,title]=titles[type]||['MELODIC ISLES','Menu'];$('#modalEyebrow').textContent=eye;$('#modalTitle').textContent=title;
  ({market:renderMarket,breed:renderBreed,collection:renderCollection,quests:renderQuests,coinrush:renderCoinRush,arcade:renderArcade,settings:renderSettings,map:renderMap,mastery:renderMastery}[type]||renderMarket)();
}
function closeModal(){if(rushGame&&rushGame.active)endCoinRush(false,true);if(arcadeGame&&arcadeGame.active)endResourceGame(false,true);$('#modalLayer').classList.add('hidden');$$('.bottom-nav button').forEach(b=>b.classList.toggle('active',b.dataset.nav==='island'));}
function openCurrencyHelp(kind){openModal('arcade');setTimeout(()=>{const map={coins:'coinGarden',gems:'gemMine',notes:'noteJam',stars:'starfall'},el=document.querySelector(`[data-arcade-card="${map[kind]||'coinGarden'}"]`);el?.scrollIntoView({behavior:'smooth',block:'center'});el?.classList.add('arcade-pulse');setTimeout(()=>el?.classList.remove('arcade-pulse'),900);},80);}
...
function renderMarket(){
  ...
  MONSTERS.filter(m=>m.island===isl).forEach(m=>{const card=document.createElement('article');card.className=`monster-card${m.special?' special-card':''}`;const owned=state.owned.filter(o=>o.id===m.id).length;card.innerHTML=`<div class="card-art">${monsterSVG(m,m.special?'Legendary':'Common')}</div><h3>${m.name}</h3>${m.special?'<span class="special-ribbon">⚙️ ISLAND TITAN</span>':''}<p>${m.desc}</p><div class="card-meta"><span class="tag">${m.element}</span><span class="tag">🎼 ${m.instrument}</span><span class="tag">${m.special?'⚙ Awaken':`⏱ ≤${Math.min(10,m.wait)}m`}</span></div><div class="mini">${fmt(m.rate)} coins/hour · Owned ${owned}</div><div class="price-row"><div class="price-stack"><b>🪙 ${fmt(m.price)}</b>${m.special?`<span>🎵 ${fmt(m.notes)} · ⭐ ${m.stars}</span>`:''}</div><button class="primary-btn">${m.special?'Awaken':'Buy'}</button></div>`;card.querySelector('button').addEventListener('click',()=>buyMonster(m));grid.appendChild(card);});
}
...
function instantBreed(job){const left=Math.max(0,job.endsAt-now()),cost=finishGemCost(left);if(state.gems<cost){toast(`You need ${cost} Gems.`);return;}state.gems-=cost;state.stats.gemsSpent+=cost;state.stats.instantBreeds++;job.endsAt=now();sfx('gem');toast(`Breeding finished instantly for ${cost} Gems!`);renderTop();renderBreed();renderIsland();save();}
...
function rotatingQuests(){
  ...
  return [
    ...
    {id:`r${rid}06`,icon:'🎮',name:'Arcade Runner',desc:'Complete 1 Resource Arcade game during this rotation.',target:1,kind:'deltaArcade',reward:{coins:2500,gems:4,notes:75,xp:75}},
...
function rotatingProgress(q){
  ...
  if(q.kind==='deltaRush')return (s.coinRushRuns||0)-(b.coinRushRuns||0);
  if(q.kind==='deltaArcade')return (s.arcadeRuns||0)-(b.arcadeRuns||0);
...
function renderCoinRush(){
  ...
  root.innerHTML=`<button class="secondary-btn arcade-back" id="rushBackArcade">← Resource Arcade</button><section class="rush-hero"><div><span class="eyebrow">5 RUNS PER ROLLING HOUR</span><h3>🪙 Coin Rush</h3><p>Find the called monster before the beat expires. Every round gets faster. Correct taps build combo and speed bonuses; wrong taps cost score. Difficulty islands unlock with your matching main islands.</p></div><div class="rush-limit"><b id="rushReadyCount">${st.remaining}/5</b><span>runs ready</span><small id="rushNextText">${st.remaining? 'A run is consumed when you press Start.' : `Next run ${formatTime(st.nextAt-now())}`}</small></div></section><div class="rush-mode-grid" id="rushModes"></div><div class="rush-history"><b>🏦 Coin Rush lifetime earnings: 🪙 ${fmt(state.coinRush.totalEarned||0)}</b><span id="rushResetLine">${rushResetText()}</span></div>`;
  $('#rushBackArcade')?.addEventListener('click',()=>openModal('arcade'));
  ...
  Object.entries(RUSH_MODES).forEach(([key,m])=>{
    ...
    c.innerHTML=`<div class="rush-island-preview"><span>${m.icon}</span><i>♪</i><i>🪙</i></div><span class="rush-mode-tag">${m.tag}</span><h3>${m.name} · ${m.rushIsland}</h3><p>${m.desc}</p><div class="rush-mode-stats"><span>🎯 ${m.rounds} targets</span><span>⚡ ${m.startMs/1000}s → ${m.endMs/1000}s</span><span>🪙 ~${fmt(low)}–${fmt(high)}</span></div><div class="rush-best">Best payout: 🪙 ${fmt(best)}</div><button class="primary-btn" data-rush-start="${key}" ${!unlocked||st.remaining<=0?'disabled':''}>${!unlocked?`Unlock ${ISLANDS[m.unlock].name} first`:st.remaining<=0?'Hourly limit reached':`Start ${m.name}`}</button>`;
    c.querySelector('button').addEventListener('click',()=>startCoinRush(key));grid.appendChild(c);
  });
}
function updateRushMenuTimer(){
  const st=rushStatus(),ready=$('#rushReadyCount'),next=$('#rushNextText'),line=$('#rushResetLine');
  if(ready)ready.textContent=`${st.remaining}/5`;
  if(next)next.textContent=st.remaining?'A run is consumed when you press Start.':`Next run ${formatTime(st.nextAt-now())}`;
  if(line)line.textContent=rushResetText();
  $$('[data-rush-start]').forEach(b=>{const key=b.dataset.rushStart,m=RUSH_MODES[key],unlocked=state.unlocked.includes(m.unlock);b.disabled=!unlocked||st.remaining<=0;b.textContent=!unlocked?`Unlock ${ISLANDS[m.unlock].name} first`:st.remaining<=0?'Hourly limit reached':`Start ${m.name}`;});
}
...
function rushClick(id,button){
  const g=rushGame;if(!g?.active||g.locked)return;if(id!==g.targetId){g.locked=true;clearRushTimers();g.misses++;g.combo=0;g.score=Math.max(0,g.score-Math.round(g.mode.hitCoins*.35));button.classList.add('wrong');sfx('bad');setTimeout(nextRushRound,180);return;}
...
function cleanArcadeRuns(key){
  const cfg=RESOURCE_GAMES[key],slot=state.arcade[key],cutoff=now()-cfg.windowMs;
  slot.runs=(slot.runs||[]).filter(t=>t>cutoff).sort((a,b)=>a-b);
}
function arcadeStatus(key){
  const cfg=RESOURCE_GAMES[key];cleanArcadeRuns(key);const slot=state.arcade[key],used=slot.runs.length,remaining=Math.max(0,cfg.limit-used),nextAt=used>=cfg.limit?slot.runs[0]+cfg.windowMs:0;return{used,remaining,nextAt};
}
function renderArcade(){
  if(arcadeGame&&arcadeGame.active){renderResourceGame();return;}
  const root=$('#modalContent');root.innerHTML=`<section class="arcade-hero"><div><span class="eyebrow">PLAY FOR REAL RESOURCES</span><h3>🎮 Resource Arcade</h3><p>Coin Rush stays as the high-skill jackpot game. These new games give you easier ways to earn Coins, Gems, Stars and Music Notes without waiting on passive income.</p></div><div class="arcade-wallet"><span>🪙 ${fmt(state.coins)}</span><span>💎 ${fmt(state.gems)}</span><span>🎵 ${fmt(state.notes)}</span><span>⭐ ${fmt(state.stars)}</span></div></section><div class="arcade-grid" id="arcadeGrid"></div>`;
  const grid=$('#arcadeGrid');
  const rush=rushStatus(),rushCard=document.createElement('article');rushCard.className='arcade-card arcade-coins';rushCard.dataset.arcadeCard='coinRush';rushCard.innerHTML=`<div class="arcade-card-icon">🪙⚡</div><span class="rush-mode-tag">HARD COIN GAME</span><h3>Coin Rush</h3><p>The original reaction-speed mode. Huge payouts, but it gets brutally fast.</p><div class="arcade-reward">Jackpots: 🪙 thousands</div><div class="arcade-limit">${rush.remaining}/5 runs ready${rush.remaining?'':` · next ${formatTime(rush.nextAt-now())}`}</div><button class="primary-btn" id="openCoinRush">Open Coin Rush</button>`;rushCard.querySelector('button').addEventListener('click',()=>openModal('coinrush'));grid.appendChild(rushCard);
  Object.entries(RESOURCE_GAMES).forEach(([key,g])=>{
    const st=arcadeStatus(key),unlocked=state.unlocked.includes(g.unlock),slot=state.arcade[key],card=document.createElement('article');card.className=`arcade-card arcade-${g.currency}${unlocked?'':' locked'}`;card.dataset.arcadeCard=key;
    const payout=key==='coinGarden'?'~4,000–9,500 Coins':key==='gemMine'?'~4–12 Gems':key==='starfall'?'~1–5 Stars':'~100–420 Notes';
    card.innerHTML=`<div class="arcade-card-icon">${g.icon}</div><span class="rush-mode-tag">${g.tag}</span><h3>${g.name}</h3><p>${g.desc}</p><div class="arcade-reward">Typical reward: ${g.rewardIcon} ${payout}</div><div class="arcade-best">${g.bestLabel}: ${g.rewardIcon} ${fmt(slot.best||0)}</div><div class="arcade-limit" data-arcade-limit="${key}">${unlocked?(st.remaining?`${st.remaining}/${g.limit} plays ready`:`Next play ${formatTime(st.nextAt-now())}`):`Unlock ${ISLANDS[g.unlock].name}`}</div><button class="primary-btn" data-arcade-start="${key}" ${!unlocked||st.remaining<=0?'disabled':''}>${!unlocked?'Locked':st.remaining<=0?'Hourly limit reached':'Play'}</button>`;
    card.querySelector('button').addEventListener('click',()=>startResourceGame(key));grid.appendChild(card);
  });
}
function updateArcadeMenuTimers(){
  Object.entries(RESOURCE_GAMES).forEach(([key,g])=>{const el=$(`[data-arcade-limit="${key}"]`),btn=$(`[data-arcade-start="${key}"]`),st=arcadeStatus(key),unlocked=state.unlocked.includes(g.unlock);if(el)el.textContent=unlocked?(st.remaining?`${st.remaining}/${g.limit} plays ready`:`Next play ${formatTime(st.nextAt-now())}`):`Unlock ${ISLANDS[g.unlock].name}`;if(btn){btn.disabled=!unlocked||st.remaining<=0;btn.textContent=!unlocked?'Locked':st.remaining<=0?'Hourly limit reached':'Play';}});
}
function startResourceGame(key){
  const cfg=RESOURCE_GAMES[key],st=arcadeStatus(key);if(!cfg)return;if(!state.unlocked.includes(cfg.unlock)){toast(`Unlock ${ISLANDS[cfg.unlock].name} first.`);return;}if(st.remaining<=0){toast(`${cfg.name} is out of plays. Next one in ${formatTime(st.nextAt-now())}.`);return;}
  state.arcade[key].runs.push(now());save();ensureAudio()?.resume?.();sfx('rushstart');
  if(key==='coinGarden')arcadeGame={active:true,key,round:0,rounds:20,hits:0,misses:0,combo:0,maxCombo:0,score:0,targetId:null,timeout:null,locked:false};
  if(key==='gemMine')arcadeGame={active:true,key,round:0,rounds:10,hits:0,misses:0,target:null,choices:[],showing:true,timeout:null,locked:false};
  if(key==='starfall')arcadeGame={active:true,key,round:0,rounds:8,hits:0,misses:0,pattern:'',choices:[],showing:true,timeout:null,locked:false};
  if(key==='noteJam')arcadeGame={active:true,key,beat:0,beats:16,score:0,perfect:0,great:0,misses:0,pos:0,dir:1,lastFrame:performance.now(),locked:false};
  nextResourceRound();
}
function nextResourceRound(){
  const g=arcadeGame;if(!g?.active)return;clearResourceTimers();
  if(g.key==='coinGarden')return nextCoinGardenRound();
  if(g.key==='gemMine')return nextGemMineRound();
  if(g.key==='starfall')return nextStarfallRound();
  if(g.key==='noteJam')return renderNoteJam();
}
function randomRegularMonster(){const pool=MONSTERS.filter(m=>!m.special&&state.unlocked.includes(m.island));return pool[Math.floor(Math.random()*pool.length)]||M.spriggle;}
function nextCoinGardenRound(){
  const g=arcadeGame;if(g.round>=g.rounds){endResourceGame(true,false);return;}g.round++;g.locked=false;g.targetId=randomRegularMonster().id;renderResourceGame();g.timeout=setTimeout(()=>coinGardenMiss(null),4500);
}
function renderCoinGarden(){
  const g=arcadeGame,target=M[g.targetId],pool=[target];let candidates=MONSTERS.filter(m=>!m.special&&m.id!==target.id&&state.unlocked.includes(m.island));while(pool.length<4&&candidates.length){const i=Math.floor(Math.random()*candidates.length);pool.push(candidates.splice(i,1)[0]);}pool.sort(()=>Math.random()-.5);const root=$('#modalContent');
  root.innerHTML=`<section class="resource-play coin-garden"><div class="resource-hud"><div><span class="eyebrow">🌼 EASY COIN GAME</span><h3>Tap the monster holding the 🪙!</h3></div><div><b>Round ${g.round}/${g.rounds}</b><span>🪙 ${fmt(g.score)}</span></div></div><div class="garden-grid" id="gardenGrid"></div><div class="rush-footer"><span>✅ ${g.hits}</span><span>❌ ${g.misses}</span><span>🔥 ×${g.combo}</span><button class="secondary-btn" id="resourceQuit">Quit</button></div></section>`;
  const grid=$('#gardenGrid');pool.forEach(m=>{const b=document.createElement('button');b.className='garden-monster';const correct=m.id===g.targetId;b.innerHTML=`<div class="garden-art">${monsterSVG(m,'Common')}${correct?'<span class="coin-crown">🪙</span>':''}</div><b>${m.name}</b>`;b.addEventListener('click',()=>correct?coinGardenHit(b):coinGardenWrong(b));grid.appendChild(b);});bindResourceQuit();
}
function coinGardenHit(btn){const g=arcadeGame;if(!g?.active||g.locked)return;g.locked=true;clearResourceTimers();g.hits++;g.combo++;g.maxCombo=Math.max(g.maxCombo,g.combo);const earned=220+Math.min(12,g.combo)*18;g.score+=earned;btn?.classList.add('correct');sfx('rushhit');if(btn){const r=btn.getBoundingClientRect();spark(r.left+r.width/2,r.top+r.height/2,'🪙');}setTimeout(nextCoinGardenRound,180);}
function coinGardenWrong(btn){const g=arcadeGame;if(!g?.active||g.locked||!btn)return;btn.classList.add('wrong');sfx('tap');setTimeout(()=>btn.classList.remove('wrong'),180);}
function coinGardenMiss(btn){const g=arcadeGame;if(!g?.active||g.locked)return;g.misses++;g.combo=0;sfx('bad');if(btn){if(btn.classList.contains('wrong'))return;g.score=Math.max(0,g.score-45);btn.classList.add('wrong');return;}g.locked=true;clearResourceTimers();setTimeout(nextCoinGardenRound,220);}
function nextGemMineRound(){
  const g=arcadeGame;if(g.round>=g.rounds){endResourceGame(true,false);return;}g.round++;g.locked=false;const gems=['💎','🔷','🔹','🟦','🧊','🔮'];g.target=gems[Math.floor(Math.random()*gems.length)];g.choices=[g.target];while(g.choices.length<4){const x=gems[Math.floor(Math.random()*gems.length)];if(!g.choices.includes(x))g.choices.push(x);}g.choices.sort(()=>Math.random()-.5);g.showing=true;renderResourceGame();g.timeout=setTimeout(()=>{if(!arcadeGame?.active)return;g.showing=false;renderResourceGame();g.timeout=setTimeout(()=>gemMineMiss(null),5000);},1150);
}
function renderGemMine(){const g=arcadeGame,root=$('#modalContent');root.innerHTML=`<section class="resource-play gem-mine"><div class="resource-hud"><div><span class="eyebrow">💎 GEM MINE</span><h3>${g.showing?'Memorize this crystal':'Which crystal did you see?'}</h3></div><b>Round ${g.round}/${g.rounds}</b></div><div class="memory-target">${g.showing?g.target:'❔'}</div><div class="memory-grid ${g.showing?'disabled':''}" id="memoryGrid">${g.choices.map(x=>`<button ${g.showing?'disabled':''} data-gem="${x}">${x}</button>`).join('')}</div><div class="rush-footer"><span>✅ ${g.hits}</span><span>❌ ${g.misses}</span><button class="secondary-btn" id="resourceQuit">Quit</button></div></section>`;$$('[data-gem]',root).forEach(b=>b.addEventListener('click',()=>b.dataset.gem===g.target?gemMineHit(b):gemMineMiss(b)));bindResourceQuit();}
function gemMineHit(btn){const g=arcadeGame;if(!g?.active||g.locked||g.showing)return;g.locked=true;clearResourceTimers();g.hits++;btn?.classList.add('correct');sfx('gemwin');setTimeout(nextGemMineRound,240);}
function gemMineMiss(btn){const g=arcadeGame;if(!g?.active||g.locked||g.showing)return;g.locked=true;clearResourceTimers();g.misses++;btn?.classList.add('wrong');sfx('bad');setTimeout(nextGemMineRound,280);}
function makeConstellation(len){const chars=['✦','★','✧','✺','⋆'];let out='';for(let i=0;i<len;i++)out+=chars[Math.floor(Math.random()*chars.length)];return out;}
function mutatePattern(p){const chars=['✦','★','✧','✺','⋆'],a=[...p],i=Math.floor(Math.random()*a.length);let x=a[i];while(x===a[i])x=chars[Math.floor(Math.random()*chars.length)];a[i]=x;return a.join('');}
function nextStarfallRound(){
  const g=arcadeGame;if(g.round>=g.rounds){endResourceGame(true,false);return;}g.round++;g.locked=false;g.pattern=makeConstellation(3+Math.floor((g.round-1)/2));g.choices=[g.pattern];while(g.choices.length<3){const x=mutatePattern(g.pattern);if(!g.choices.includes(x))g.choices.push(x);}g.choices.sort(()=>Math.random()-.5);g.showing=true;renderResourceGame();g.timeout=setTimeout(()=>{if(!arcadeGame?.active)return;g.showing=false;renderResourceGame();g.timeout=setTimeout(()=>starfallMiss(null),6000);},1400);
}
function renderStarfall(){const g=arcadeGame,root=$('#modalContent');root.innerHTML=`<section class="resource-play starfall"><div class="resource-hud"><div><span class="eyebrow">⭐ STARFALL OBSERVATORY</span><h3>${g.showing?'Study the constellation':'Pick the constellation you saw'}</h3></div><b>Round ${g.round}/${g.rounds}</b></div><div class="constellation-show">${g.showing?g.pattern:'☁️'}</div><div class="constellation-grid">${g.choices.map((x,i)=>`<button ${g.showing?'disabled':''} data-star-choice="${i}">${x}</button>`).join('')}</div><div class="rush-footer"><span>✅ ${g.hits}</span><span>❌ ${g.misses}</span><button class="secondary-btn" id="resourceQuit">Quit</button></div></section>`;$$('[data-star-choice]',root).forEach(b=>b.addEventListener('click',()=>g.choices[Number(b.dataset.starChoice)]===g.pattern?starfallHit(b):starfallMiss(b)));bindResourceQuit();}
function starfallHit(btn){const g=arcadeGame;if(!g?.active||g.locked||g.showing)return;g.locked=true;clearResourceTimers();g.hits++;btn?.classList.add('correct');sfx('starwin');setTimeout(nextStarfallRound,260);}
function starfallMiss(btn){const g=arcadeGame;if(!g?.active||g.locked||g.showing)return;g.locked=true;clearResourceTimers();g.misses++;btn?.classList.add('wrong');sfx('bad');setTimeout(nextStarfallRound,300);}
function renderNoteJam(){
  const g=arcadeGame;if(g.beat>=g.beats){endResourceGame(true,false);return;}const root=$('#modalContent');root.innerHTML=`<section class="resource-play note-jam"><div class="resource-hud"><div><span class="eyebrow">🎵 NOTE JAM</span><h3>Hit the glowing center on beat</h3></div><div><b>Beat ${g.beat+1}/${g.beats}</b><span>🎵 ${fmt(g.score)}</span></div></div><div class="note-lane"><div class="sweet-zone"></div><div class="note-marker" id="noteMarker">♪</div></div><button class="note-hit-btn" id="noteHit">HIT BEAT</button><div class="note-grade" id="noteGrade">Perfect zone = biggest Note payout</div><div class="rush-footer"><span>✨ ${g.perfect} perfect</span><span>✅ ${g.great} great</span><span>❌ ${g.misses} miss</span><button class="secondary-btn" id="resourceQuit">Quit</button></div></section>`;$('#noteHit').addEventListener('click',noteJamHit);bindResourceQuit();g.pos=0;g.dir=1;g.locked=false;g.lastFrame=performance.now();cancelAnimationFrame(arcadeRAF);arcadeRAF=requestAnimationFrame(noteJamFrame);
}
function noteJamFrame(t){const g=arcadeGame;if(!g?.active||g.key!=='noteJam')return;const dt=Math.min(40,t-g.lastFrame);g.lastFrame=t;g.pos+=g.dir*dt*.075;if(g.pos>=100){g.pos=100;g.dir=-1;}if(g.pos<=0){g.pos=0;g.dir=1;}const marker=$('#noteMarker');if(marker)marker.style.left=`calc(${g.pos}% - 18px)`;arcadeRAF=requestAnimationFrame(noteJamFrame);}
function noteJamHit(){const g=arcadeGame;if(!g?.active||g.locked)return;g.locked=true;cancelAnimationFrame(arcadeRAF);const dist=Math.abs(g.pos-50),grade=$('#noteGrade');let add=0;if(dist<=8){add=22;g.perfect++;if(grade)grade.textContent='PERFECT ✨ +22';sfx('notehit');}else if(dist<=18){add=14;g.great++;if(grade)grade.textContent='GREAT! +14';sfx('rushhit');}else if(dist<=30){add=7;g.great++;if(grade)grade.textContent='GOOD +7';sfx('tap');}else{g.misses++;if(grade)grade.textContent='MISS';sfx('bad');}g.score+=add;g.beat++;setTimeout(renderNoteJam,420);}
function renderResourceGame(){if(!arcadeGame?.active){renderArcade();return;}if(arcadeGame.key==='coinGarden')renderCoinGarden();else if(arcadeGame.key==='gemMine')renderGemMine();else if(arcadeGame.key==='starfall')renderStarfall();else if(arcadeGame.key==='noteJam')renderNoteJam();}
function bindResourceQuit(){$('#resourceQuit')?.addEventListener('click',()=>{if(confirm('Quit this arcade run? The play will still count toward the hourly limit.'))endResourceGame(false,false);});}
function clearResourceTimers(){if(!arcadeGame)return;if(arcadeGame.timeout){clearTimeout(arcadeGame.timeout);arcadeGame.timeout=null;}cancelAnimationFrame(arcadeRAF);arcadeRAF=null;}
function endResourceGame(completed=false,silent=false){
  if(!arcadeGame)return;const g=arcadeGame,cfg=RESOURCE_GAMES[g.key];clearResourceTimers();g.active=false;if(!completed){arcadeGame=null;if(!silent){toast(`${cfg.name} run ended.`);renderArcade();}return;}
  let reward=0,detail='';
  if(g.key==='coinGarden'){reward=g.score+Math.round((g.hits/g.rounds)*1800);state.coins+=reward;state.stats.collectedCoins+=reward;state.stats.coinGardenRuns=(state.stats.coinGardenRuns||0)+1;detail=`${g.hits}/${g.rounds} coins caught · best combo ×${g.maxCombo}`;gainXP(Math.max(12,Math.round(reward/300)));}
  if(g.key==='gemMine'){reward=Math.max(2,Math.round(g.hits*.8)+(g.hits===g.rounds?4:0));state.gems+=reward;state.stats.gemMineRuns=(state.stats.gemMineRuns||0)+1;detail=`${g.hits}/${g.rounds} crystals remembered`;}
  if(g.key==='starfall'){reward=Math.max(1,Math.floor(g.hits/2)+(g.hits===g.rounds?1:0));grantStars(reward);state.stats.starfallRuns=(state.stats.starfallRuns||0)+1;detail=`${g.hits}/${g.rounds} constellations remembered`;}
  if(g.key==='noteJam'){reward=Math.max(45,g.score+60);state.notes+=reward;state.stats.noteJamRuns=(state.stats.noteJamRuns||0)+1;detail=`${g.perfect} perfect · ${g.great} good/great · ${g.misses} misses`;}
  state.stats.arcadeRuns=(state.stats.arcadeRuns||0)+1;const slot=state.arcade[g.key];slot.best=Math.max(slot.best||0,reward);slot.total=(slot.total||0)+reward;save();renderTop();sfx(g.key==='gemMine'?'gemwin':g.key==='starfall'?'starwin':'arcadewin');
  const left=arcadeStatus(g.key).remaining,root=$('#modalContent');root.innerHTML=`<section class="resource-results arcade-${cfg.currency}"><div class="rush-result-icon">${cfg.icon}</div><span class="eyebrow">${cfg.name}</span><h3>Run complete!</h3><div class="resource-payout">+ ${cfg.rewardIcon} ${fmt(reward)}</div><p>${detail}</p><div class="rush-result-grid"><span>Best ever<b>${cfg.rewardIcon} ${fmt(slot.best)}</b></span><span>Lifetime earned<b>${cfg.rewardIcon} ${fmt(slot.total)}</b></span><span>Plays left<b>${left}/${cfg.limit}</b></span></div><div class="result-actions"><button class="primary-btn" id="resourceAgain">Play again</button><button class="secondary-btn" id="resourceBack">Resource Arcade</button></div></section>`;
  $('#resourceAgain')?.addEventListener('click',()=>{const key=g.key;arcadeGame=null;if(arcadeStatus(key).remaining>0)startResourceGame(key);else renderArcade();});$('#resourceBack')?.addEventListener('click',()=>{arcadeGame=null;renderArcade();});
}
...
function sfx(k){if(!state.settings.sfx)return;ensureAudio()?.resume?.();const map={tap:[380,.05,'sine'],coin:[860,.08,'triangle'],buy:[520,.12,'triangle'],breed:[330,.2,'sine'],hatch:[660,.3,'triangle'],quest:[900,.25,'sine'],level:[980,.3,'sine'],unlock:[740,.3,'triangle'],bad:[150,.14,'square'],gem:[1180,.18,'sine'],titan:[392,.65,'sine'],rushstart:[520,.22,'triangle'],rushhit:[980,.07,'sine'],rushwin:[760,.35,'triangle'],gemwin:[1180,.28,'sine'],starwin:[920,.4,'triangle'],notehit:[690,.08,'sine'],arcadewin:[840,.3,'triangle']};const v=map[k]||map.tap;tone(v[0],v[1],v[2],.055,0,'sfx');if(['hatch','quest','level','unlock','titan','rushstart','rushwin','gemwin','starwin','arcadewin'].includes(k))tone(v[0]*1.5,v[1]*1.3,'sine',.032,.08,'sfx');if(k==='titan')tone(v[0]*2,.9,'sine',.025,.14,'sfx');}
