...
function freshState(){
  return {
    ...
    stats:{collectedCoins:0,breedCount:0,decorBought:0,songsPlayed:0,instantBreeds:0,notesSpent:0,starsSpent:0,gemsSpent:0,monstersBought:0,masteryBought:0,hatched:0},
...
let state=load();
let audio=null,masterGain=null,musicBus=null,sfxBus=null,songTimer=null,songStep=0,selectedBreed=[],collectionVariant='All';
...
function demoState(){
  ...
  s.stats={...s.stats,collectedCoins:98000,breedCount:18,decorBought:9,songsPlayed:14,instantBreeds:3,notesSpent:740,starsSpent:11,gemsSpent:14,monstersBought:25,masteryBought:5,hatched:16};
  s.breedJobs.groove={startedAt:now()-60000,endsAt:now()+4*60000,monsterId:'spriggle',variant:'Legendary'};
  s.rotationBaseline={...s.stats};s.rotationBaseline.collectedCoins-=1500;s.rotationBaseline.songsPlayed-=1;
  return s;
}
...
function init(){
  ensureRotationState();buildParticles();bindStatic();grantDaily();checkFinishedJobs(false);awardCollectionMilestones(false);renderAll();
  setInterval(()=>{ensureRotationState();updateTimers();renderTop();if(!$('#modalLayer').classList.contains('hidden')&&$('#modalTitle').textContent==='Quests')renderQuests();},1000);
  window.addEventListener('beforeunload',save);
  document.addEventListener('visibilitychange',()=>{if(document.hidden)save();else{ensureRotationState();checkFinishedJobs(true);renderAll();}});
  const p=new URLSearchParams(location.search),screen=(DEMO&&DEMO.screen)||p.get('screen'),island=(DEMO&&DEMO.island)||p.get('island');
  if(demoMode){if(island&&ISLANDS[island])state.currentIsland=island;renderAll();if(screen&&['market','breed','collection','quests','settings','map','mastery'].includes(screen))setTimeout(()=>openModal(screen),100);}
}
function bindStatic(){
...
function openModal(type){
  ...
  const titles={market:['MONSTER MARKET','Market'],breed:['HARMONY HATCH','Breeding'],collection:['MONSTER BOOK','Collection'],quests:['GOALS','Quests'],settings:['OPTIONS','Settings'],map:['WORLD','Island Map'],mastery:['STAR WORKSHOP','Island Mastery']};const [eye,title]=titles[type]||['MELODIC ISLES','Menu'];$('#modalEyebrow').textContent=eye;$('#modalTitle').textContent=title;
  ({market:renderMarket,breed:renderBreed,collection:renderCollection,quests:renderQuests,settings:renderSettings,map:renderMap,mastery:renderMastery}[type]||renderMarket)();
}
function closeModal(){$('#modalLayer').classList.add('hidden');$$('.bottom-nav button').forEach(b=>b.classList.toggle('active',b.dataset.nav==='island'));}
...
function rotatingQuests(){const rid=state.rotationId,seed=(rid*9301+49297)%233280,level=state.level;return [
  {id:`r${rid}a`,icon:'🪙',name:'Four-Hour Coin Run',desc:`Collect ${fmt(1200+level*180)} coins during this rotation.`,target:1200+level*180,kind:'deltaCoins',reward:{notes:60+level*2,gems:2,xp:55}},
  {id:`r${rid}b`,icon:'🎶',name:'Four-Hour Encore',desc:'Play 2 complete island performances.',target:2,kind:'deltaSongs',reward:{stars:1,notes:80,xp:65}},
  {id:`r${rid}c`,icon:'💞',name:seed%2?'Hatch Rush':'Breeding Session',desc:'Complete 1 breeding during this rotation.',target:1,kind:'deltaBreed',reward:{gems:4,coins:1600+level*120,xp:70}}
];}
function rotatingProgress(q){const b=state.rotationBaseline||state.stats;if(q.kind==='deltaCoins')return state.stats.collectedCoins-(b.collectedCoins||0);if(q.kind==='deltaSongs')return state.stats.songsPlayed-(b.songsPlayed||0);if(q.kind==='deltaBreed')return state.stats.breedCount-(b.breedCount||0);return 0;}
function rotationEndsIn(){return ((state.rotationId+1)*FOUR_HOURS)-Date.now();}
function renderQuests(){
  ensureRotationState();const root=$('#modalContent');root.innerHTML=`<div class="quest-section-title"><div><h3>🔄 Rotating Quests <span class="quest-badge">3 ACTIVE</span></h3><div class="mini">Refresh automatically every 4 hours, even while the game is closed.</div></div><span class="refresh-clock">${formatTime(rotationEndsIn())}</span></div><div id="rotatingList"></div><div class="quest-section-title"><h3>🏆 Main Questbook <span class="quest-badge">20 QUESTS</span></h3><button class="secondary-btn" id="dailyBtn">🎁 Daily gift</button></div><div id="questList"></div>`;
...
function questCard(q,p,done,claimed,rot,fn){const c=document.createElement('article');c.className=`quest-card${done?' done':''}${rot?' refresh':''}`;c.innerHTML=`<div class="quest-icon">${q.icon}</div><div class="quest-copy"><h3>${q.name}${rot?'<span class="quest-badge">4H</span>':''}</h3><p>${q.desc}</p><div class="progress-track" style="margin-top:8px"><div class="progress-fill" style="width:${clamp(p/q.target*100,0,100)}%"></div></div><div class="mini">${fmt(Math.min(p,q.target))} / ${fmt(q.target)}</div><div class="reward-line">${rewardText(q.reward)}</div></div><button class="primary-btn" ${!done||claimed?'disabled':''}>${claimed?'Claimed':'Claim'}</button>`;c.querySelector('button').addEventListener('click',fn);return c;}
...
function sfx(k){if(!state.settings.sfx)return;ensureAudio()?.resume?.();const map={tap:[380,.05,'sine'],coin:[860,.08,'triangle'],buy:[520,.12,'triangle'],breed:[330,.2,'sine'],hatch:[660,.3,'triangle'],quest:[900,.25,'sine'],level:[980,.3,'sine'],unlock:[740,.3,'triangle'],bad:[150,.14,'square'],gem:[1180,.18,'sine'],titan:[392,.65,'sine']};const v=map[k]||map.tap;tone(v[0],v[1],v[2],.055,0,'sfx');if(['hatch','quest','level','unlock','titan'].includes(k))tone(v[0]*1.5,v[1]*1.3,'sine',.032,.08,'sfx');if(k==='titan')tone(v[0]*2,.9,'sine',.025,.14,'sfx');}
